...
const ElectromagnetismLab: React.FC<Partial<SimulationProps>> = ({ theme = 'light' }) => {
  // --- State ---
  ...
  return (
    <div className={`flex flex-col gap-6 w-full max-w-4xl mx-auto p-6 font-sans border rounded-2xl shadow-sm select-none transition-colors duration-500 ${
      ...
      <div className="text-center">
        <h2 className={`text-2xl font-bold m-0 ${theme === 'dark' ? 'text-white' : 'text-slate-800'}`}>Electromagnetic Induction Lab</h2>
        <p className={`text-sm mt-1 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>Move the magnet through the coil to generate electricity (Faraday's Law)</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className={`lg:col-span-2 p-4 rounded-xl border shadow-sm flex flex-col items-center relative overflow-hidden transition-colors duration-500 ${
          ...
          <div className={`w-full flex justify-between items-center px-2 pb-2 border-b ${theme === 'dark' ? 'border-slate-800' : 'border-slate-100'}`}>
            <span className={`text-xs font-bold uppercase tracking-wider flex items-center gap-1 ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`}>
              <Zap size={14} className="text-amber-500" /> Physical Viewport
            </span>
            <span className={`text-xs font-mono font-bold ${theme === 'dark' ? 'text-slate-300' : 'text-slate-600'}`}>
              Coils: {numCoils} | v: {Math.abs(velocity).toFixed(0)} px/s
            </span>
          </div>

          <svg viewBox="0 0 400 300" className={`w-full h-auto my-4 rounded-lg cursor-grab active:cursor-grabbing transition-colors duration-500 ${
            theme === 'dark' ? 'bg-slate-900' : 'bg-slate-50/50'
          }`}>
          ...
          <div className="w-full px-8 pb-4">
            <label className={`flex items-center justify-center gap-2 text-xs font-bold mb-2 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>
            ...
            <input 
              ...
              className={`w-full accent-slate-700 ${theme === 'dark' ? 'accent-sky-500' : 'accent-slate-700'}`}
            />
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          {/* Voltmeter Display */}
          <div className={`p-5 rounded-xl border shadow-lg text-center transition-colors duration-500 ${
            theme === 'dark' ? 'bg-slate-950 border-slate-800' : 'bg-slate-900 border-slate-700'
          }`}>
            <span className={`text-[10px] font-bold uppercase tracking-widest block mb-1 ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`}>Galvanometer (EMF)</span>
          ...
          {/* Settings */}
          <div className={`p-4 rounded-xl border space-y-4 transition-colors duration-500 ${
            theme === 'dark' ? 'bg-slate-950 border-slate-800' : 'bg-white border-slate-200'
          }`}>
            <div>
              <label className={`text-xs font-bold block mb-2 uppercase ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`}>Coil Density (N)</label>
                ...
                {[2, 3, 5].map(n => (
                  <button 
                    ...
                    className={`flex-1 py-2 text-xs font-bold rounded-lg border transition-all ${
                      numCoils === n 
                        ? theme === 'dark' ? 'bg-sky-500 text-white border-sky-500 shadow-[0_0_15px_rgba(14,165,233,0.3)]' : 'bg-indigo-600 text-white border-indigo-600 shadow-md' 
                        : theme === 'dark' ? 'bg-slate-900 text-slate-400 border-slate-800 hover:bg-slate-800' : 'bg-slate-100 text-slate-600 border-slate-200 hover:bg-slate-200'
                    }`}
                  >
                    {n} Turns
                  </button>
          ...
          <div className={`p-4 rounded-xl border h-full flex flex-col min-h-[180px] transition-colors duration-500 ${
            theme === 'dark' ? 'bg-slate-950 border-slate-800' : 'bg-white border-slate-200'
          }`}>
            <span className={`text-[10px] font-bold uppercase tracking-wider mb-2 flex items-center gap-1 ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`}>
              <Activity size={14} /> Voltage Waveform
            </span>
            
            <div className={`flex-1 flex items-center gap-0.5 border-l border-b relative overflow-hidden ${
              theme === 'dark' ? 'border-slate-800 bg-slate-900/50' : 'border-slate-100 bg-slate-50/30'
            }`}>
              
              {/* Zero-line reference */}
              <div className={`absolute w-full h-[1px] top-1/2 left-0 z-0 ${theme === 'dark' ? 'bg-slate-800' : 'bg-slate-200'}`}></div>
      ...
      {/* Theory Card */}
      <div className={`p-4 rounded-xl flex gap-3 border transition-colors duration-500 ${
        ...
        <div className={`text-xs leading-relaxed ${theme === 'dark' ? 'text-blue-200' : 'text-blue-800'}`}>
