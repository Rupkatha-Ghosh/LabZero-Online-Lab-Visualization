...
const ChatPanel: React.FC<ChatPanelProps> = ({ userName }) => {
  ...
  return (
    <aside className="flex h-full min-h-0 flex-col rounded-2xl bg-[#303134] sm:rounded-[24px]">
      ...
      <div className="flex gap-2 border-t border-white/10 p-3 sm:p-4">
        ...
        />
        <button onClick={sendMessage} className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-sky-600 text-white hover:bg-sky-500">
          <Send size={18} />
        </button>
      </div>
    </aside>
